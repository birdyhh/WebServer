...
ssize_t Buffer::ReadFd(int fd, int *saveError)
{
    ...
    //从文件fd中读取数据，存到iov[2]中的buf1和buf2，返回读到的总字节数
    ...
    {//读取的总字节数没有超过iov[0]的缓存范围
        writePos_ += len;
    }
    else
    {//读取的总字节数超过iov[0]的缓存范围
...
void Buffer::MakeSpace_(size_t len)
{
    ...
    else
    {
        ...
        std::copy(BeginPtr_() + readPos_, BeginPtr_() + writePos_,
                  BeginPtr_());
        readPos_ = 0;
